...
<div class="row" id="purchaseReturn">
    <div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;">
        <div class="form-group" style="margin-top:10px;">
            <label class="col-sm-1 col-sm-offset-1 control-label no-padding-right" for="purchaseInvoiceno"> Invoice no
            </label>
            <div class="col-sm-3">
                <v-select v-bind:options="invoices" label="invoice_text" v-model="selectedInvoice"
                    v-bind:disabled="purchaseReturn.returnId == 0 ? false : true"></v-select>
            </div>
            <div class="col-sm-2">
                <button class="btn btn-info btn-xs" style="width: 100px;" @click="getPurchaseDetailsForReturn"
                    v-bind:disabled="purchaseReturn.returnId == 0 ? false : true">View</button>
            </div>
        </div>
    </div>
    <div class="col-xs-12 col-md-12 col-lg-12" v-if="cart.length > 0" style="display:none"
        v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
        <br>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr v-for="(product, sl) in cart">
                        <td>{{ sl + 1 }}</td>
                        <td>{{ product.Product_Name }} - {{product.Article_No}}</td>
                        ...
                        <td><input type="text" v-model="product.return_quantity" v-on:input="productReturnTotal(sl)">
                        </td>
    ...
    methods: {
        ...
        async productReturnTotal(ind) {
            if (this.checkStock) {
                ...
                let returnDetail = this.returnDetails.find(rd => rd.PurchaseReturnDetailsProduct_SlNo ==
                    this.cart[ind].Product_IDNo);
            ...
            if (this.cart[ind].return_quantity > (this.cart[ind].PurchaseDetails_TotalQuantity - this.cart[
                    ind].returned_quantity)) {
            ...
            this.cart[ind].return_amount = parseFloat(this.cart[ind].return_quantity) * parseFloat(this
                .cart[ind].return_rate);
            this.calculateTotal();
        },
        calculateTotal() {
        ...
        savePurchaseReturn() {
            let filteredCart = this.cart.filter(product => product.return_quantity > 0 && product.return_rate >
                0);
        ...
        getReturn() {
            axios.post('/get_purchase_returns', {
                ...
                this.cart.map(product => {
                    let returnDetail = this.returnDetails.find(rd => rd
                        .PurchaseReturnDetailsProduct_SlNo == product.Product_IDNo);
                    product.return_quantity = returnDetail
                        ?.PurchaseReturnDetails_ReturnQuantity;
                    product.returned_quantity = +product.returned_quantity - (returnDetail
                        ?.PurchaseReturnDetails_ReturnQuantity ?? 0);
                    product.return_amount = returnDetail
                        ?.PurchaseReturnDetails_ReturnAmount;
                    product.returned_amount = +product.returned_amount - (returnDetail
                        ?.PurchaseReturnDetails_ReturnAmount ?? 0);
                    return product;
                })
            })
        }
