...
<div class="row" id="salesReturn">
        ...
        <form class="form-inline" id="searchForm">
            <div class="form-group" style="display:none;" v-bind:style="{display: customers.length > 0 ? '' : 'none'}">
                ...
                <v-select v-bind:options="customers" label="display_name" v-model="selectedCustomer"
                    v-on:input="getInvoices" v-bind:disabled="salesReturn.returnId == 0 ? false : true"></v-select>
            </div>

            <div class="form-group">
                <label>Invoice</label>
                <v-select v-bind:options="invoices" label="SaleMaster_InvoiceNo" v-model="selectedInvoice"
                    v-bind:disabled="salesReturn.returnId == 0 ? false : true"></v-select>
            </div>
            <div class="form-group">
                <button type="button" class="btn btn-info btn-xs" style="width: 100px;" @click="getSaleDetailsForReturn"
                    v-bind:disabled="salesReturn.returnId == 0 ? false : true">View</button>
            </div>
        </form>
    </div>
    <div style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
        <div class="col-xs-12 col-md-12 col-lg-12">
            <br>
            <div class="col-md-6">
                Return date: <input type="date" v-model="salesReturn.returnDate"
                    v-bind:disabled="userType == 'u' ? true : false"><br><br>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr v-for="(product, sl) in cart">
                                ...
                                <td><input type="text" v-model="product.return_quantity"
                                        v-on:input="productReturnTotal(sl)"></td>
                                <td><input type="text" v-model="product.return_rate"
                                        v-on:input="productReturnTotal(sl)"></td>
...
new Vue({
    ...
    methods: {
        ...
        productReturnTotal(ind) {
            if (this.cart[ind].return_quantity > (this.cart[ind].SaleDetails_TotalQuantity - this.cart[ind]
                    .returned_quantity)) {
            ...
            this.cart[ind].return_amount = parseFloat(this.cart[ind].return_quantity) * parseFloat(this.cart[
                ind].return_rate);
            this.calculateTotal();
        },
        calculateTotal() {
        ...
        async getReturn() {
            ...
            this.cart.map(product => {
                let returnDetail = returnData.returnDetails.find(rd => rd
                ...
                product.returned_quantity = product.returned_quantity - (returnDetail
                    ?.SaleReturnDetails_ReturnQuantity ?? 0);
                ...
                product.returned_amount = product.returned_amount - (returnDetail
                    ?.SaleReturnDetails_ReturnAmount ?? 0);
                return product;
            })
