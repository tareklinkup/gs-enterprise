                ...
                <table class="table table-bordered">
                    ...
                    <tbody style="display:none;" v-bind:style="{display: transfers.length > 0 ? '' : 'none'}">
                        <tr v-for="(transfer, sl) in transfers">
                            ...
                            <td>{{ transfer.note }}</td>
                            <td>
                                <a href="" v-bind:href="`/transfer_invoice/${transfer.transfer_id}`" target="_blank"
                                    title="View invoice"><i class="fa fa-file"></i></a>
                                <a v-if="transfer.status == 'p'" href="" title="Approve"
                                    @click.prevent="approveItem(transfer)"><i style="font-size:18px"
                                        class="fa fa-check-square"></i> Approve </a>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
new Vue({
    ...
    methods: {
        ...
        approveItem(transfer) {
            let deleteConf = confirm('Are you sure to approve?');
            if (deleteConf == false) {
                return;
            }
            axios.post('/approve_transfer', {
                    transfer: transfer
                })
                .then(res => {
                    let r = res.data;
