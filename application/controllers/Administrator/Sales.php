...
class Sales extends CI_Controller {
    ...
    public function getSales(){
        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != ''){
            ...
            $size_color = $this->db->query("
            select
            sd.size_id,
            sd.color_id
            from tbl_saledetails sd 
            where sd.SaleMaster_IDNo = '$data->salesId'
            ")->row();


            if($size_color->color_id != 0 && $size_color->size_id != 0){
                $saleDetails = $this->db->query("
                select 
                    sd.*,
                    p.Product_Code,
                ...
                join tbl_size s on s.Size_SlNo = sd.size_id
                join tbl_color c on c.color_SiNo = sd.color_id
                where sd.SaleMaster_IDNo = ?
            ", $data->salesId)->result();
    
            $res['saleDetails'] = $saleDetails;
            }
            elseif( $size_color->size_id != 0)
            {
                $saleDetails = $this->db->query("
                select 
                    sd.*,
...
<?php
    ...
    public function  deleteSales(){
        ...
        try{
            ...
            $saleDetails = $this->db->select('Product_IDNo, size_id, color_id, SaleDetails_TotalQuantity')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();
