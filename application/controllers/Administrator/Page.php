...
class Page extends CI_Controller {
     ...
     public function size()
    {
        ...
        $data['title'] = "Add Size";
        $data['content'] = $this->load->view('Administrator/size', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function insert_size()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $succ =  $this->mt->save_data('tbl_size', $data);
            if ($succ) {
                $msg = true;
                echo json_encode($msg);
            }
  ...
  public function sizeedit($id)
    {
        ...
        $data['content'] = $this->load->view('Administrator/edit/size_edit', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
...
// Get Size
    ...
    {
        $units = $this->db->query("select s.*, s.Size_Name as Display_Size from tbl_size s where status = 'a'")->result();
        echo json_encode($units);
    }
    ...
    public function getColors(){
        $colors = $this->db->query("select c.*, c.color_name as display_color from tbl_color c where status = 'a'")->result();
