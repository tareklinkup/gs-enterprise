...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        ...
        try{
            ...
            $articleNumberCount = $this->db->query("select * from tbl_product where article_no = ?", $productObj->article_no)->num_rows();
            if($articleNumberCount > 0){
                $res = ['success'=>false, 'message'=>'Article Number already exists'];
                echo json_encode($res);
                exit;
            }
    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Name, ' - ', p.Article_No) as display_text,
    ...
    public function getTotalStock(){
        ...
        // if(isset($data->size_id) && $data->size_id != null){
        //     $clauses .= " and s.Size_SlNo = '$data->size_id'";
        // }

        // if(isset($data->color_id) && $data->color_id != null){
        //     $clauses .= " and c.color_SiNo = '$data->color_id'";
        // }
